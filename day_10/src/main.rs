...
fn sum_trailhead_scores(map_size: (i32, i32), map: &[u8]) -> usize {
	(0..map_size.0)
		...
		.map(|trailhead_start| get_trailhead_score(trailhead_start, 0, map_size, map))
		.map(|trailhead_ends| trailhead_ends.len())
		.sum()
}

fn get_trailhead_score(curr_pos: (i32, i32), current_height: u8, map_size: (i32, i32), map: &[u8]) -> HashSet<(i32, i32)> {
	...
	[(0,1), (1, 0), (0, -1), (-1, 0)].iter()
		...
		.flat_map(|new_pos| get_trailhead_score(new_pos, new_height, map_size, map))
		.collect()
}
